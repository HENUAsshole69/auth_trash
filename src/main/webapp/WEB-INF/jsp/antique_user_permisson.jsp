				...
				<div class="permission">
					<form class="layui-form" action="${pageContext.request.contextPath}/antique/user/updatePermission">
						...
						<div style="margin-top: 30px;">
							...
							<input type="checkbox" name="roleBox" value="文物局" title="文物局" id="antique" <c:if test="${role.antique == 'yes'}">checked</c:if>  lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="老化痕迹鉴定人员" title="老化痕迹鉴定人员" <c:if test="${role.burn_in == 'yes'}">checked</c:if> id="burn_in"  lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="工作人员" title="工作人员" id="worker" <c:if test="${role.worker == 'yes'}">checked</c:if> lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="文物商店" title="文物商店" id="antique_store" <c:if test="${role.antique_store == 'yes'}">checked</c:if> lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="拍卖行" title="拍卖行" id="auction" <c:if test="${role.auction == 'yes'}">checked</c:if> lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="普通用户" class="ordinary" id="ordinary" <c:if test="${role.ordinary == 'yes'}">checked</c:if> title="普通用户" lay-skin="primary"><br/><br/>
							<input type="checkbox" name="roleBox" value="财务人员" title="财务人员" id="finance" <c:if test="${role.finance == 'yes'}">checked</c:if> lay-skin="primary"><br/><br/>
							<button type="submit" class="layui-btn layui-btn-warm layui-btn-sm" style="position: relative;left: 350px;">立即提交</button>
						</div>
					</form>
				</div>
			</div>
